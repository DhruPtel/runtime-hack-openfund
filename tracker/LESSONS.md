...
**This file is history, not authority.** [PLAN.md](../PLAN.md),
[ROADMAP.md](../ROADMAP.md) and [PHASE-0-1.md](../PHASE-0-1.md) are the
...
**Affects:** Phase 5 (renamed and rescoped), probe 0.5, PLAN.md §13, the demo story.
...
Cadence was open in PLAN.md §12 and blocked `config/cadence.json`, the scheduler
...
CODEBASE.md §2 and the bare `PHASE-0-1.md` reference in ROADMAP.md, and unit 0.1
builds the skeleton, so building against the wrong layout would have guaranteed a
later move. Docs moved to the repository root, `Research/` renamed to
`research/`, and the package placed at `src/fund/` per CODEBASE.md §2, which wins
over PLAN.md §7's bare `src/`.
...
**Affects:** 1.5, 1.6, 3.4, 3.8; PLAN.md §9; `config/thresholds.json`.
...
as one.
**Affects:** 0.4, 1.4, 3.4; PLAN.md §11.
...
while PLAN.md §6 specifies it off and nothing in the system calls
...
and is stated in PLAN.md §13.
**Affects:** 1.3, 1.9, 1.10; PLAN.md §13.
...
serving book, trades, mirror and signals endpoints. Removed from ROADMAP.md
...
being first.
**Affects:** ROADMAP.md 7.5, README.md, 8.6.
...
**Affects:** PLAN.md §2; 2.5, 3.7, 3.9; `config/models.json`.

## 2026-09-17 — New unit 4.12: treasurer as its own process
PLAN.md invariant 1 requires the treasurer to run as its own process, proven by
an environment test rather than an import graph, and CODEBASE.md §3 calls that
...
submission before 4.12 passes), PLAN.md §5.
...
stock fills are out of scope and stated in PLAN.md §13. Unit 5.6 still runs, and
the 403 is now the expected result for the stock path rather than an error case.
**Affects:** Phase 5 name, units 5.1–5.7 and its exit; PLAN.md §13; 8.4 and 8.6,
