...
| **2.3** Brief · L | One versioned file per seat, `briefs/<seat>.v1.md`, plus a shared contract, `briefs/analyst.v1.md`. The seat's question and the questions it leaves to others come from `analysts.json`; add the rules 0.9's brief states, the schema, and the snapshot's bytes. Every seat sees the whole universe and differs in question. *(A file per seat since the 2.2–2.5 batch brief, 2026-09-19.)* | Mandate, explicit scope boundaries, snapshot bytes, schema, effort scaling; assets partitioned in config. | Asset partitioning and effort scaling. Overlap is prevented by question only. |
...
| **2.5** Tokens · L | Each call records the response's `usage` block and its cost at the listed price, labelled an estimate. So do the per-analyst and per-cycle totals. The aggregate is checked against a settled `/v1/usage` window, with the window's dates read back from the provider. **Never a before-and-after delta,** of credits or of usage (F0.9.2). *(Changed by the 2.2–2.5 batch brief, 2026-09-19; the earlier minimal version used per-agent credit balances.)* | Per-call counts reconciled to `/v1/usage` in settled windows, aggregate to aggregate (F0.6.4, F0.9.2). | Per-call confirmation, which the provider cannot give (F0.6.4), and per-agent balance deltas. |
...
|---|---|---|---|
| **3.1** Aggregator · L, invariant 6 tested | A total function, as in "The report format". Below quorum, or with every seat abstaining: no rebalance, holdings kept. Positive scores become weights, capped at the position limit; the remainder goes to cash, ties break by address, and the residual is shown. *(Changed by the Phase 3 batch brief, operator 2026-09-19: a target starts at the weight held. A buy raises it by score × the position limit, a sell cuts it, and hold, silence and caution alone keep it; a new position comes only from a buy. The earlier line gave a held asset on hold a weight of 0, and the planner would have sold it. Built: `core/aggregate.py`.)* | Equation, confidence scale, horizon, quorum, cash weight, tie-break, rounding residual. | Horizon, and weighting seats unequally. Confidence is taken at face value. |
...
| **3.6** Context budget · L | The bundle is measured before the call, conservatively from its bytes, because the gateway has no token-count endpoint (F1.7.2). Over budget is a veto, never a summary. *(Built before 3.5, at the operator's order, 2026-09-19: the risk call needs the check before it is sent.)* | An exact budget over reports, plan and risk output (invariant 3). | An exact count. The estimate errs toward vetoing. |
...
| **4.10 ▶** Single-owner lock · **H** | **Rescoped** (the operator, 2026-09-19): one lock row, and a second runner refuses; a row left by a dead process on this host is taken over. The crash drill it was is covered by the kill drills at 4.3 and 4.8. | A live kill mid-submission, and a lease with a heartbeat. | A live kill, and any lock that survives the host. |
...
| **6.3** Cost · L | Per agent, the sum of its calls' estimates from 2.5, with the aggregate checked against a settled `/v1/usage` window. *(Was: from its own account's balance; changed with 2.5, 2026-09-19.)* | Estimate flags and pricing basis, reconciled to `/v1/usage` in settled windows. | Settled-window reconciliation. |
...
| `thresholds.quorum_min_analysts` | 3 (set at 3.1) | One failed seat still decides. |
| `thresholds.max_position_weight` | 0.25 (set at 3.1) | Two $25 orders in one name at $200. |
| `thresholds.cash_floor_usd` | 20 (set at 3.1) | 10% of the paper capital. |
| `thresholds.turnover_max_bps` | 10,000 (set at 3.4) | A cycle may trade at most its paper NAV. Loose, and stated as loose. |
| `thresholds.min_order_usd` (new) | 1 (set at 3.3) | Drops rebalancing dust, so an unchanged view means no trade. |
| `mandate.cumulative_budget_usd` | 5 | Real money on the live leg only. |
| `mandate.live_leg_usd` (new) | 0.50 | Fits what the wallet holds. |
| `mandate.allowed_assets` | the 35 markable stocks (paper), ETH and USDG (live). *Provisional since 3.4: the capture's 20 tradeable stocks, until 4.1.* | The snapshot decides which are tradeable each cycle. |
| `mandate.approved_by`, `approved_at`, `expires_at` | the operator, at approval, plus 7 days. *Placeholders since 3.4, which 4.1 replaces.* | 4.1 refuses nulls. |
